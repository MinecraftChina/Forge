...
import net.minecraftforge.fml.common.FmlReflectionAPI;
...
public class ModDiscoverer
{
        ...
        else
        {
            for (int i = 0; i < minecraftSources.length; i++)
            {
                if (minecraftSources[i].isFile())
                {
                    ...
                    else if (false == FmlReflectionAPI.checkPermission(getClass().getClassLoader(),  minecraftSources[i])) {//add by yfg。检查官方mod的md5.
    ...
    //add by yfg.检查mod权限。
    public void checkModDirMods(File modsDir)
    {
